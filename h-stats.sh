...
#echo $gpu_temp
№CUSTOM_API_TIMEOUT=1
№CUSTOM_API_PORT=4050

#threads=`echo "threads" | nc -w $CUSTOM_API_TIMEOUT localhost ${CUSTOM_API_PORT}` && echo $threads
threads=`echo "threads" | nc -w $CUSTOM_API_TIMEOUT localhost ${CUSTOM_API_PORT} | tr -d '\0'` && echo $threads
if [[ $? -ne 0  || -z $threads ]]; then
	echo -e "${YELLOW}Failed to read $miner stats from localhost:${CUSTOM_API_PORT}${NOCOLOR}"
else
	summary=`echo "summary" | nc -w $CUSTOM_API_TIMEOUT localhost ${CUSTOM_API_PORT} | tr -d '\0'`
